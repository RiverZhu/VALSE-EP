function [Zhat, Zvar] = GaussianMomentsComputation_MJH(Y, Phat,Pvar,yy_min, Quantize_bit,Quantize_stepsize, Var)
    % This code is originally written by Jianhua Mo
    m = length(Y);
	Mean = 0;
    ...
    if ~isempty(I)
    Zhat(I) = Phat(I) + (Pvar(I) ./ (Pvar(I) + Var + 1/12 * (Quantize_stepsize)^2 )).*...
        ( Mean + 0.5*(t_lower(I) + t_upper(I)) - Phat(I));  %% I made a mistake in this line before!!!

    Zvar(I) = Pvar(I).^2 ./ (Pvar(I) + Var + 1/12 * (Quantize_stepsize)^2 );
    end;
...
%     end;
    ...
    Zhat(I1) = Phat(I1) - ( (Pvar(I1) ./ sqrt(Pvar(I1) + Var)).* Ratio_upper(I1) );

    Zhat(I2) = Phat(I2) + ( (Pvar(I2) ./ sqrt(Pvar(I2) + Var)).* Ratio_lower(I2) );

    Zvar(I1) = Pvar(I1) - (Pvar(I1).^2 ./ (Pvar(I1) + Var)) .*...
    Ratio_upper(I1).*(-z_upper(I1) + Ratio_upper(I1));
    Zvar(I2) = Pvar(I2) - (Pvar(I2).^2 ./ (Pvar(I2) + Var)) .*...
